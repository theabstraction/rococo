...
namespace
{
	...
	public:
		...
		virtual void RenderGui(IGuiRenderContext& grc)
		{
			...
			Vec2i introSpan = RenderTextVCentred(grc, centre.x, (int32)topLeft.y, RGBAb{ 0xFF, 0xFF, 0xFF, 0xFF }, title.c_str(), 3);

			int32 buttonHeight = 60;

			GuiRect targetRect(rect.left, rect.top + introSpan.y, rect.right, rect.bottom - buttonHeight);
			LeftAlignedText body(targetRect, retzone, hypzone, 7, message.c_str(), FontColourFromRGBAb(RGBAb{ 0xFF, 0xFF, 0xFF, 0xFF }));
