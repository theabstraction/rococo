...
namespace ANON
{
    ...
    struct Users_Table_Implementation: IUsersSupervisor, IUsers_MetaData
    {
        ...
        const IUsers_Row* FindRowByOwnerId(fstring ownerId) const override
        {
            auto i = ownerId_to_index.find(ownerId);
            return i != ownerId_to_index.end() ? &i->second : nullptr;
        }

        const IUsers_Row* FindRowByPurchaseId(int64 purchaseId) const override
        {
            auto i = purchaseId_to_index.find(purchaseId);
            return i != purchaseId_to_index.end() ? &i->second : nullptr;
        }
