// Generated by rococo.carpenter. Timestamp: 19/06/2022 21:09:12
...
{
    if (!source || !*source) source = "!tables/rococo.periodic-table.Elements_Table.bin";
...
namespace ANON
{
    ...
    struct Elements_Table_Implementation: IPeriodicTableSupervisor, IPeriodicTable_MetaData
    {
        Rococo::Science::Materials::IPeriodicTable_Sexy& GetSexyInterface()
        {
            return *this;
        }

        std::vector<PeriodicTableRow> rows;
        ...
        void GetRow(int32 index, PeriodicTableRow& row) override
        {
            if (index > NumberOfRows()) Throw(0, "%s: [index] out of range.", __FUNCTION__);
            row = GetRow(index);
        }
        ...
        {
            rows.clear();
            AppendPeriodicTableRows(rows, installation, tablePingPath);
        }
...
Rococo::Science::Materials::IPeriodicTable_Sexy* FactoryConstructRococoScienceMaterialsGetPeriodicTable(IInstallation* installation)
{
...
namespace Rococo::Science::Materials
{
    ...
    std::tuple<boolean32,ElementName> TryParseElementName(const fstring& text)
    {
    ...
    std::tuple<boolean32,ElementSymbol> TryParseElementSymbol(const fstring& text)
    {
    ...
    std::tuple<boolean32,ElementType> TryParseElementType(const fstring& text)
    {
