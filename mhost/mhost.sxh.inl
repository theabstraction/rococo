...
// BennyHill generated Sexy native functions for MHost::ICamera 
namespace
{
	using namespace Rococo;
	using namespace Rococo::Sex;
	...
	void NativeMHostICameraSetPosition(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		Vec3* position;
		...
		MHost::ICamera* _pObject;
		_offset += sizeof(_pObject);

		ReadInput(_pObject, _sf, -_offset);
		_pObject->SetPosition(*position);
	}
	void NativeMHostICameraSetOrientation(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		MHost::WorldOrientation* wo;
		_offset += sizeof(wo);
		ReadInput(wo, _sf, -_offset);

		MHost::ICamera* _pObject;
		_offset += sizeof(_pObject);

		ReadInput(_pObject, _sf, -_offset);
		_pObject->SetOrientation(*wo);
	}
	void NativeMHostICameraSetPerspective(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		float far;
		...
		ReadInput(_pObject, _sf, -_offset);
		_pObject->SetPerspective(near, far);
	}
	void NativeMHostICameraGetWorldToCamera(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		Matrix4x4* m;
		...
		ReadInput(_pObject, _sf, -_offset);
		_pObject->GetWorldToCamera(*m);
	}
	void NativeMHostICameraGetCameraToScreen(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		Matrix4x4* m;
		...
		ReadInput(_pObject, _sf, -_offset);
		_pObject->GetCameraToScreen(*m);
	}

}

namespace MHost { 
	void AddNativeCalls_MHostICamera(Rococo::Script::IPublicScriptSystem& ss, MHost::ISceneBuilder* _nceContext)
	{
		const INamespace& ns = ss.AddNativeNamespace(("MHost.Native"));
		ss.AddNativeCall(ns, NativeMHostICameraSetPosition, nullptr, ("ICameraSetPosition (Pointer hObject)(Sys.Maths.Vec3 position) -> "));
		ss.AddNativeCall(ns, NativeMHostICameraSetOrientation, nullptr, ("ICameraSetOrientation (Pointer hObject)(MHost.WorldOrientation wo) -> "));
		ss.AddNativeCall(ns, NativeMHostICameraSetPerspective, nullptr, ("ICameraSetPerspective (Pointer hObject)(Float32 near)(Float32 far) -> "));
		ss.AddNativeCall(ns, NativeMHostICameraGetWorldToCamera, nullptr, ("ICameraGetWorldToCamera (Pointer hObject)(Sys.Maths.Matrix4x4 m) -> "));
		ss.AddNativeCall(ns, NativeMHostICameraGetCameraToScreen, nullptr, ("ICameraGetCameraToScreen (Pointer hObject)(Sys.Maths.Matrix4x4 m) -> "));
	}
}
// BennyHill generated Sexy native functions for MHost::ISceneBuilder 
namespace
{
	using namespace Rococo;
	using namespace Rococo::Sex;
	...
	void NativeMHostISceneBuilderCamera(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		...
		ReadInput(_pObject, _sf, -_offset);
		MHost::ICamera* camera = _pObject->Camera();
		_offset += sizeof(CReflectedClass*);
		auto& _cameraStruct = Rococo::Helpers::GetDefaultProxy(("MHost"),("ICamera"), ("ProxyICamera"), _nce.ss);
		CReflectedClass* _sxycamera = _nce.ss.Represent(_cameraStruct, camera);
		WriteOutput(&_sxycamera->header.pVTables[0], _sf, -_offset);
	}
	void NativeMHostISceneBuilderCreateCubeTexture(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		_offset += sizeof(IString*);
		...
		ReadInput(_pObject, _sf, -_offset);
		IdTexture cubeId = _pObject->CreateCubeTexture(folder, extension);
		_offset += sizeof(cubeId);
		WriteOutput(cubeId, _sf, -_offset);
	}
	void NativeMHostISceneBuilderSetSkyBox(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		IdTexture cubeId;
...
namespace MHost { 
	void AddNativeCalls_MHostISceneBuilder(Rococo::Script::IPublicScriptSystem& ss, MHost::ISceneBuilder* _nceContext)
	{
		const INamespace& ns = ss.AddNativeNamespace(("MHost.Native"));
...
namespace
{
	...
	void NativeMHostIEngineScene(NativeCallEnvironment& _nce)
	{
		Rococo::uint8* _sf = _nce.cpu.SF();
		ptrdiff_t _offset = 2 * sizeof(size_t);
		MHost::IEngine* _pObject;
		...
		ReadInput(_pObject, _sf, -_offset);
		MHost::ISceneBuilder* scene = _pObject->Scene();
		_offset += sizeof(CReflectedClass*);
		auto& _sceneStruct = Rococo::Helpers::GetDefaultProxy(("MHost"),("ISceneBuilder"), ("ProxyISceneBuilder"), _nce.ss);
		CReflectedClass* _sxyscene = _nce.ss.Represent(_sceneStruct, scene);
		WriteOutput(&_sxyscene->header.pVTables[0], _sf, -_offset);
	}
...
namespace MHost { 
	void AddNativeCalls_MHostIEngine(Rococo::Script::IPublicScriptSystem& ss, MHost::IEngine* _nceContext)
	{
		...
		ss.AddNativeCall(ns, NativeMHostIEngineScene, nullptr, ("IEngineScene (Pointer hObject) -> (MHost.ISceneBuilder scene)"));
	}
}
