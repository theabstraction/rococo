...
{
    public enum FQTypeCategory
    {
        IsStruct,
        IsEnum
    }

    public struct FQType: IComparable<FQType>
    {
        public FQTypeCategory category;
        public string ns;
        public string typeName;

        public static string ToString(FQTypeCategory cat)
        {
            switch(cat)
            {
                case FQTypeCategory.IsStruct: return "struct";
                case FQTypeCategory.IsEnum: return "enum";
                default:
                    throw new ArgumentException("Unknown category");
            }
        }

        public static bool operator == (FQType a, FQType b)
        {
            return a.category == b.category && a.ns == b.ns && a.typeName == b.typeName;
        }
        ...
        {
            if (obj.GetType() == typeof(FQType))
            {
                return ((FQType)obj) == this;
            }
            return false;
        }
        public override int GetHashCode()
        {
            return category.GetHashCode() ^ ns.GetHashCode() ^ typeName.GetHashCode();
        }

        public int CompareTo(FQType other)
        {
            int v = ns.CompareTo(other.ns);
            if (v != 0) return v;

            v = typeName.CompareTo(other.typeName);
            return v;
        }
    }
    ...
    public static class CPPCore
    {
        ...
        static HashSet<FQType> declarations = new HashSet<FQType>();

        public static void AddFQType(FQTypeCategory category, string ns, string typeName)
        {
            declarations.Add(new FQType() { category = category, ns = ns, typeName = typeName });
        }
        ...
        public static void Commit()
        {
            ...
            FQType[] elements = new FQType[declarations.Count];
            declarations.CopyTo(elements);
            Array.Sort(elements);

            StringBuilder sb = new StringBuilder();
            sb.AppendLine("//Rococo.Carpenter CPPGenerator Declarations");

            var t = DateTime.Now;
            sb.AppendFormat("// Generated At: {0} {1}", t.ToLongDateString(), t.ToLongTimeString());
            sb.AppendLine();
            sb.AppendLine();
            ...
            foreach (var e in elements)
            {
                ...
                sb.AppendLine(e.ns);
                sb.AppendLine("{");
                sb.AppendFormat("\t{0} {1};", FQType.ToString(e.category), e.typeName);
                sb.AppendLine();
                sb.AppendLine("}");
                sb.AppendLine();
            }

            File.WriteAllText(Environment.DeclarationsFullPath, sb.ToString());
        }
    }
    ...
    public class CPPGenerator : IParsedColumnData
    {
        ...
        public void AppendHeaderIncludes()
        {
            ...
            var t = DateTime.Now;
            sb.AppendFormat("{0} {1}", t.ToLongDateString(), t.ToLongTimeString());
            sb.Append("// Excel Source: ");
            sb.AppendLine(CPPCore.ExcelSource);
            sb.AppendLine();

            sb.AppendFormat("#include <{0}>", Environment.DeclarationsHeader);
            sb.AppendLine();
        ...
        public void AppendEnumDef(StringBuilder sb, string enumName)
        {
            ...
            AppendTab(sb);
            sb.AppendFormat("tuple<boolean32,{0}> TryParse{0}(const fstring& text);", enumName);
            sb.AppendLine();
        ...
        public static void AppendCPPString(StringBuilder sb, string item)
        {
            ...
            sb.AppendFormat("u8R\"{0}({1}){0}\"", raw, item.Replace("\n", "\r\n"));
        }
        ...
        public void AppendFactoryConstruct(StringBuilder sb)
        {
            ...
            sb.Append("(Rococo::IInstallation* installation)");
        }
        ...
        */
        ...
        public void AppendTypeFunctions()
        {
            ...
            foreach(var def in CPPCore.EnumDefs)
            {
                ...
                sb.AppendLine();
                AppendTab(sb);
                sb.AppendFormat("tuple<boolean32,{0}> TryParse{0}(const fstring& text)", def.Key);
                sb.AppendLine();
                AppendTab(sb);
                sb.AppendLine("{");
                AppendTab(sb);
            ...
            if (Environment.TargetConfig.AdditionalSourceHeaders != null)
            {
                foreach (string header in Environment.TargetConfig.AdditionalSourceHeaders)
                {
