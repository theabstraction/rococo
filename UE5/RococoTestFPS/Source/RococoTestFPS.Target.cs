...
public class RococoTestFPSTarget : TargetRules
{
	...
	private string GetConfigItem(string item, string configText, string source)
	{
		...
		int newLineIndex = configText.IndexOf('\r');
		...
		return configText.Substring(i + item.Length, endIndex - i);
	}
    ...
    private void CopyRococoContentToUE5Content(TargetInfo target)
	{
		...
		string packageRelPath = GetConfigItem("Package.Content=", configText, rococoContentCfg);
		...
		foreach(string subDir in contentDirectoriesToCopy)
		{
			...
			foreach (string fullDevFile in devFiles)
			{
				...
				string packageFile = Path.Combine(packageRelPath, devFile);

				Console.WriteLine(string.Format("Copying {0} to {1}...", devFile, packageFile));
				File.Copy(devFile, packageFile, true);
			}
		}
    }
