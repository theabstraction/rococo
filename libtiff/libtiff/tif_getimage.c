...
TIFFRGBAImageOK(TIFF* tif, char emsg[1024])
{
	...
	if (!tif->tif_decodestatus) {
		sprintf_s(emsg, 1024, "Sorry, requested compression method is not configured");
		return (0);
	}
	switch (td->td_bitspersample) {
		...
		case 16:
			break;
		default:
			sprintf_s(emsg, 1024, "Sorry, can not handle images with %d-bit samples",
			    td->td_bitspersample);
			return (0);
	}
	...
	if (!TIFFGetField(tif, TIFFTAG_PHOTOMETRIC, &photometric)) {
		switch (colorchannels) {
			...
			case 3:
				...
				break;
			default:
				sprintf_s(emsg, 1024, "Missing needed %s tag", photoTag);
				return (0);
		}
	}
	switch (photometric) {
		...
		case PHOTOMETRIC_RGB:
			if (colorchannels < 3) {
				sprintf_s(emsg, 1024, "Sorry, can not handle RGB image with %s=%d",
		...
		case PHOTOMETRIC_LOGL:
			if (td->td_compression != COMPRESSION_SGILOG) {
				sprintf_s(emsg, 1024, "Sorry, LogL data must have %s=%d",
		...
		case PHOTOMETRIC_LOGLUV:
			if (td->td_compression != COMPRESSION_SGILOG &&
				...
				sprintf_s(emsg, 1024, "Sorry, LogLuv data must have %s=%d or %d",
				    "Compression", COMPRESSION_SGILOG, COMPRESSION_SGILOG24);
				return (0);
			}
			if (td->td_planarconfig != PLANARCONFIG_CONTIG) {
				sprintf_s(emsg, 1024, "Sorry, can not handle LogLuv images with %s=%d",
		...
		case PHOTOMETRIC_CIELAB:
			break;
		default:
			sprintf_s(emsg, 1024, "Sorry, can not handle image with %s=%d",
			    photoTag, photometric);
			return (0);
	}
	return (1);
...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	switch (img->photometric) {
		case PHOTOMETRIC_PALETTE:
			if (!TIFFGetField(tif, TIFFTAG_COLORMAP,
				...
				sprintf_s(emsg, 1024, "Missing required \"Colormap\" tag");
                                goto fail_return;
			}
			...
			if( !img->redcmap || !img->greencmap || !img->bluecmap ) {
				sprintf_s(emsg, 1024, "Out of memory for colormap copy");
                                goto fail_return;
			}
		...
		case PHOTOMETRIC_SEPARATED:
			{
				...
				TIFFGetFieldDefaulted(tif, TIFFTAG_INKSET, &inkset);
				if (inkset != INKSET_CMYK) {
					sprintf_s(emsg, 1024, "Sorry, can not handle separated image with %s=%d",
					    "InkSet", inkset);
                                        goto fail_return;
				}
	...
	if (img->isContig) {
		if (!PickContigCase(img)) {
			sprintf_s(emsg, 1024, "Sorry, can not handle image");
			goto fail_return;
		}
	} else {
