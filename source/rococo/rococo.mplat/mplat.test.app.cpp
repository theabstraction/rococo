...
struct TestApp : IApp, private IScene, public IEventCallback<FileModifiedArgs>
{
   ...
   const LightConstantBuffer* GetLights(size_t& nLights) const override
   {
	   nLights = 0;
	   return nullptr;
   }
   ...
   void GetCamera(Matrix4x4& camera, Matrix4x4& worldToCamera, Vec4& eye, Vec4& viewDir)
   {
	   camera = worldToCamera = Matrix4x4::Identity();
	   eye = { 0,0,0 };
	   viewDir = { 0,0,1 };
   }
